...
def analyze_MODIS_temperature(mod_directory, shape_path):
    ...
    for i, files in enumerate(modis_file_list):
        ...
        for j, polygons in enumerate(import_list):
            ...
            stations_list.append(mean_Mod)
            print(Mod_temperature_array)
            print(mean_Mod)
        print(stations_list)
        Modis_mean_list.append(stations_list)
    print(np.rot90(np.rot90(np.rot90(Modis_mean_list))))
...
#     prints an array for the stations according to the scenes
#     :return:
